...
{
    public partial class UCReservationControlModify : UserControl
    {
        private readonly int _resrNo;

        private UCReservationControlModify() { }

        public UCReservationControlModify(int resrNo)
        {
            InitializeComponent();
            _resrNo = resrNo;
        }

        private void UCResrvationControlModify_Load(object sender, EventArgs e)
        {
            ...
            List<ReservationDTO> listReservationDTO = ((FormReservation)((Form)this.TopLevelControl).Owner).Select_ReservationDTO(_resrNo);
            foreach (ReservationDTO reservationDTO in listReservationDTO)
            {
                string[] splitDate = reservationDTO.Resr_date.Split('-');
                string[] splitStartTime = reservationDTO.Resr_start_time.Split(':');
                string[] splitEndTime = reservationDTO.Resr_end_time.Split(':');
                textBoxYear.Text = splitDate[0];
                textBoxMonth.Text = splitDate[1];
                textBoxDay.Text = splitDate[2];
                textBoxStartHour.Text = splitStartTime[0];
                textBoxStartMinute.Text = splitStartTime[1];
                textBoxStartSecond.Text = splitStartTime[2];
                textBoxEndHour.Text = splitEndTime[0];
                textBoxEndMinute.Text = splitEndTime[1];
                textBoxEndSecond.Text = splitEndTime[2];
                comboBoxGroupBroadcasting.SelectedIndex = comboBoxGroupBroadcasting.FindString(reservationDTO.Resr_group_broadcasting);
                comboBoxDong.SelectedIndex = comboBoxAudio.FindString(reservationDTO.Resr_dong);
                comboBoxStairs.SelectedIndex = comboBoxAudio.FindString(reservationDTO.Resr_stairs);
                comboBoxFloor.SelectedIndex = comboBoxAudio.FindString(reservationDTO.Resr_floor);
                comboBoxAudio.SelectedIndex = comboBoxAudio.FindString(reservationDTO.Extn_name);
                comboBoxStartContactPoint.SelectedIndex = comboBoxAudio.FindString(reservationDTO.Resr_start_contact_point);
                comboBoxEndContactPoint.SelectedIndex = comboBoxAudio.FindString(reservationDTO.Resr_end_contact_point);
            }
        }
        ...
        private void Update_ReservationDTO()
        {
            ...
            ((FormReservation)((Form)this.TopLevelControl).Owner).ModifyList.Add(prepared);
        }
        #endregion
    }
}