...
{
    public partial class UCTimerControlModify : UserControl
    {
        private readonly int _timrNo;

        private UCTimerControlModify() { }

        public UCTimerControlModify(int timrNo)
        {
            InitializeComponent();
            _timrNo = timrNo;
        }

        private void UCTimerControlModify_Load(object sender, EventArgs e)
        {
            ...
            List<TimerDTO> listTimerDTO = ((FormTimer)((Form)this.TopLevelControl).Owner).Select_TimerDTO(_timrNo);
            foreach (TimerDTO timerDTO in listTimerDTO)
            {
                string[] splitStartTime = timerDTO.Timr_start_time.Split(':');
                string[] splitEndTime = timerDTO.Timr_end_time.Split(':');
                ...
                checkBoxMon.Checked = bool.Parse(timerDTO.Timr_mon);
                checkBoxTue.Checked = bool.Parse(timerDTO.Timr_tue);
                checkBoxWed.Checked = bool.Parse(timerDTO.Timr_wed);
                checkBoxThu.Checked = bool.Parse(timerDTO.Timr_thu);
                checkBoxFri.Checked = bool.Parse(timerDTO.Timr_fri);
                checkBoxSat.Checked = bool.Parse(timerDTO.Timr_sat);
                checkBoxSun.Checked = bool.Parse(timerDTO.Timr_sun);
                comboBoxDong.SelectedIndex = comboBoxAudio.FindString(timerDTO.Timr_dong);
                comboBoxStairs.SelectedIndex = comboBoxAudio.FindString(timerDTO.Timr_stairs);
                comboBoxFloor.SelectedIndex = comboBoxAudio.FindString(timerDTO.Timr_floor);
                comboBoxAudio.SelectedIndex = comboBoxAudio.FindString(timerDTO.Extn_name);
                comboBoxStartContactPoint.SelectedIndex = comboBoxAudio.FindString(timerDTO.Timr_start_contact_point);
                comboBoxEndContactPoint.SelectedIndex = comboBoxAudio.FindString(timerDTO.Timr_end_contact_point);
            }
        }
        ...
        private void Update_TimerDTO()
        {
            ...
            ((FormTimer)((Form)this.TopLevelControl).Owner).ModifyList.Add(prepared);
        }
        #endregion
    }
}
